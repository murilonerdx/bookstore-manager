package com.murilonerdx.bookmanager.exception;

import com.murilonerdx.bookmanager.exception.dto.ApiError;
...
public class BookstoreExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  public ResponseEntity<Object>
  handleEntityExistsException(EntityExistsException exception) {
    return buildResponseEntity(
  ...
  handleMethodArgumentNotValid(MethodArgumentNotValidException exception,
    ...
    exception.getBindingResult().getFieldErrors().forEach(
        fieldError
        -> errors.add("Field " + fieldError.getField().toUpperCase() + " " +
                      fieldError.getDefaultMessage()));
    exception.getBindingResult().getGlobalErrors().forEach(
        objectError
        -> errors.add("Object " + objectError.getObjectName() + " " +
  ...
  @Override
  protected ResponseEntity<Object>
  handleHttpMessageNotReadable(HttpMessageNotReadableException exception,
                               HttpHeaders headers, HttpStatus status,
                               WebRequest request) {
    return buildResponseEntity(
