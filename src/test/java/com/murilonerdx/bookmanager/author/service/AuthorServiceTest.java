package com.murilonerdx.bookmanager.author.service;
...
public class AuthorServiceTest {
  ...
  void whenNewAuthorIsInformedThenItShouldBeCreated() {
    ...
    when(authorRepository.save(expectedCreatedAuthor))
  ...
  void whenExistingAuthorIsInformedThenAnExceptionShouldBeThrown() {
    AuthorDTO expectedAuthorToCreateDTO = authorDtoBuilder.buildAuthorDTO();
    Author expectedDuplicatedAuthor =
        authorMapper.toModel(expectedAuthorToCreateDTO);

    when(authorRepository.findByName(expectedAuthorToCreateDTO.getName()))
        .thenReturn(Optional.of(expectedDuplicatedAuthor));
  ...
  void whenValidNameIsGivenThenAnAuthorShouldBeReturned() {
    ...
    when(authorRepository.findByName(expectedFoundAuthorDTO.getName()))
        .thenReturn(Optional.of(expectedFoundAuthor));

    AuthorDTO foundAuthor =
        authorService.findByName(expectedFoundAuthorDTO.getName());
  ...
  void whenListAuthorsIsCalledThenItShouldBeReturned() {
    AuthorDTO expectedFoundAuthorDTO =
        AuthorDTOBuilder.builder().build().buildAuthorDTO();
    Author expectedFoundAuthor = authorMapper.toModel(expectedFoundAuthorDTO);
  ...
  void whenExistingAuthorIdIsGivenThenAuthorEntityShouldBeReturned() {
    ...
    Author foundAuthor =
        authorService.verifyAndGetIfExists(expectedFoundAuthorDTO.getId());
  ...
  void whenNotExistingAuthorIdIsGivenThenAndExceptionShouldBeThrown() {
    ...
    assertThrows(AuthorNotFoundException.class,
                 ()
                     -> authorService.verifyAndGetIfExists(
  ...
  void whenValidAuthorIdIsGivenThenItShouldBeReturned() {
    ...
    Author expectedDeletedAuthor =
        authorMapper.toModel(expectedDeletedAuthorDTO);
  ...
  void whenInvalidAuthorIsGivenThenAnExceptionShouldBeThrown() {
    ...
    when(authorRepository.findById(expectedNotFoundAuthorId))
        .thenReturn(Optional.empty());

    assertThrows(AuthorNotFoundException.class,
                 () -> authorService.delete(expectedNotFoundAuthorId));
  }
}
