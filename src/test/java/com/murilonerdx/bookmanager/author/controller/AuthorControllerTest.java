...
public class AuthorControllerTest {
  ...
  void whenPOSTIsCalledThenStatusCreatedShouldBeInformed() throws Exception {
    ...
    mockMvc
        .perform(post(AUTHOR_API_URL_PATH)
                     .contentType(MediaType.APPLICATION_JSON)
                     .content(asJsonString(expectedCreatedAuthorDTO)))
        .andExpect(status().isCreated())
        .andExpect(
            jsonPath("$.id", is(expectedCreatedAuthorDTO.getId().intValue())))
  ...
  void whenPOSTIsCalledWithoutRequiredFieldThenBadRequestShouldBeInformed()
      throws Exception {
    AuthorDTO expectedCreatedAuthorDTO =
  ...
  @Test
  void whenGETWithValidNameIsCalledThenOkStatusShouldBeReturned()
      throws Exception {
    AuthorDTO expectedCreatedAuthorDTO =
        AuthorDTOBuilder.builder().build().buildAuthorDTO();

    when(authorService.findByName(expectedCreatedAuthorDTO.getName()))
        .thenReturn(expectedCreatedAuthorDTO);

    mockMvc
        .perform(
            get(AUTHOR_API_URL_PATH + "/" + expectedCreatedAuthorDTO.getName())
                .contentType(MediaType.APPLICATION_JSON))
  ...
  void whenGETListIsCalledThenOkStatusShouldBeReturned() throws Exception {
    ...
    when(authorService.findAll())
        .thenReturn(Collections.singletonList(expectedCreatedAuthorDTO));

    mockMvc
        .perform(
            get(AUTHOR_API_URL_PATH).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].id",
                            is(expectedCreatedAuthorDTO.getId().intValue())))
        .andExpect(
            jsonPath("$[0].name", is(expectedCreatedAuthorDTO.getName())))
  ...
  void whenDELETEWithValidIdIsCalledThenNoContentShouldBeReturned()
    ...
    mockMvc
        .perform(
            delete(AUTHOR_API_URL_PATH + "/" + expectedCreatedAuthorDTO.getId())
                .contentType(MediaType.APPLICATION_JSON))
