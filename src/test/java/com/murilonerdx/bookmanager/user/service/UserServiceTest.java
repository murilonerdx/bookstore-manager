...
public class UserServiceTest {
  ...
  void whenExistingUserIsInformedThenItShouldBeUpdated() {
    ...
    String expectedUpdateMessage =
        "Username Murilo Update with ID 1 successfully updated";
    ...
    MessageDTO successUpdatedMessage =
        userService.update(expectedUpdatedUser.getId(), expectedUpdatedUserDTO);

    assertThat(successUpdatedMessage.getMessage(),
               is(equalTo(expectedUpdateMessage)));
  }
  ...
  void whenNotFoundUserIsInformedThenAnExceptionShouldBeThrown() {
    ...
    assertThrows(
        ...
        () -> userService.update(INVALID_USER_ID, expectedUpdatedUserDTO));
  }
  ...
  void whenValidUsernameIsInformedThenUserEntityShouldBeReturned() {
    ...
    User foundUser = userService.verifyAndGetUserIfExists(
  ...
  void whenInvalidUsernameIsInformedThenUserEntityShouldBeReturned() {
    ...
    assertThrows(UserNotFoundException.class,
                 ()
                     -> userService.verifyAndGetUserIfExists(
                         expectedFoundUserDTO.getUsername()));
  }
  ...
  void whenValidUserIsInformedThenItShouldBeDeleted() {
    ...
    when(userRepository.findById(expectedDeletedUserDTO.getId()))
