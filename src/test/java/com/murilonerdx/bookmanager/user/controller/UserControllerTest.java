package com.murilonerdx.bookmanager.user.controller;
...
import com.murilonerdx.bookmanager.model.user.controller.UserController;
...
public class UserControllerTest {
  ...
  void whenPOSTIsCalledThenCreatedShouldBeInformed() throws Exception {
    ...
    String expectedCreationMessage =
        "Username rodrigo with ID 1 successfully created";
    MessageDTO expectedCreationMessageDTO =
        MessageDTO.builder().message(expectedCreationMessage).build();

    when(userService.create(userDTO)).thenReturn(expectedCreationMessageDTO);

    mockMvc
        .perform(post(USER_API_URL_PATH)
  ...
  @Test
  void whenPOSTIsCalledTWithoutRequiredFieldThenBadRequestShouldBeInformed()
      throws Exception {
    UserDTO userDTO = userDTOBuilder.buildUserDTO();
  ...
  @Test
  void whenPOSTIsCalledToAuthenticateUserThenOkShouldBeInformed()
      throws Exception {
    JwtRequest jwtRequest = jwtRequestBuilder.buildJwtRequest();
    JwtResponse expectedJwtToken =
        JwtResponse.builder().jwtToken("testToken").build();

    when(authenticationService.createAuthenticationToken(jwtRequest))
        .thenReturn(expectedJwtToken);

    mockMvc
        .perform(post(USER_API_URL_PATH + "/authenticate")
                     .contentType(MediaType.APPLICATION_JSON)
  ...
  @Test
  void
  whenPOSTIsCalledToAuthenticateUserWithoutPasswordThenBadRequestShouldBeInformed()
      throws Exception {
    JwtRequest jwtRequest = jwtRequestBuilder.buildJwtRequest();
  ...
  void whenPUTIsCalledThenOkStatusShouldBeInformed() throws Exception {
    ...
    String expectedCreationMessage =
        "Username Rodrigo updated with ID 1 successfully updated";
    MessageDTO expectedUpdatedMessageDTO =
        MessageDTO.builder().message(expectedCreationMessage).build();
    ...
    mockMvc
        .perform(put(USER_API_URL_PATH + "/" + userDTO.getId())
                     .contentType(MediaType.APPLICATION_JSON)
                     .content(asJsonString(userDTO)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message", is(expectedCreationMessage)));
  ...
  void whenDELETEIsCalledThenNoContentShouldBeInformed() throws Exception {
    ...
    mockMvc
        .perform(delete(USER_API_URL_PATH + "/" + userDTO.getId())
                     .contentType(MediaType.APPLICATION_JSON))
