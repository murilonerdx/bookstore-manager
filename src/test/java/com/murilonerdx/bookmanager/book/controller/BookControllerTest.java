...
public class BookControllerTest {
  ...
  void whenPOSTIsCalledThenCreatedShouldBeInformed() throws Exception {
    ...
    when(bookService.create(any(AuthenticatedUser.class),
                            eq(expectedBookToCreateDTO)))
        .thenReturn(expectedCreatedBookDTO);

    mockMvc
        .perform(post(BOOKS_API_URL_PATH)
                     .contentType(MediaType.APPLICATION_JSON)
                     .content(JsonConvertionUtils.asJsonString(
                         expectedBookToCreateDTO)))
        .andExpect(status().isCreated())
        .andExpect(
            jsonPath("$.id", is(expectedBookToCreateDTO.getId().intValue())))
        .andExpect(jsonPath("$.name", is(expectedBookToCreateDTO.getName())))
        .andExpect(jsonPath("$.isbn", is(expectedBookToCreateDTO.getIsbn())));
  }

  @Test
  void whenPOSTIsCalledWithoutRequiredFieldsThenBadRequestShouldBeInformed()
      throws Exception {
    BookRequestDTO expectedBookToCreateDTO =
        bookRequestDTOBuilder.buildRequestBookDTO();
  ...
  void whenGETIsCalledThenStatusOkShouldBeInformed() throws Exception {
    BookRequestDTO expectedBookToFind =
        bookRequestDTOBuilder.buildRequestBookDTO();
    BookResponseDTO expectedCreatedBookDTO =
        bookResponseDTOBuilder.buildBookResponseDTO();

    when(bookService.findByIdAndUser(any(AuthenticatedUser.class),
                                     eq(expectedBookToFind.getId())))
        .thenReturn(expectedCreatedBookDTO);

    mockMvc
        .perform(get(BOOKS_API_URL_PATH + "/" + expectedBookToFind.getId())
                     .contentType(MediaType.APPLICATION_JSON))
  ...
  void whenGETListIsCalledThenStatusOkShouldBeInformed() throws Exception {
    ...
    mockMvc
        .perform(
            get(BOOKS_API_URL_PATH).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$[0].id", is(expectedBookToFind.getId().intValue())))
  ...
  void whenPUTIsCalledThenStatusOkShouldBeInformed() throws Exception {
    ...
    when(bookService.updateByUser(any(AuthenticatedUser.class),
                                  eq(expectedBookToUpdateDTO.getId()),
                                  eq(expectedBookToUpdateDTO)))
        .thenReturn(expectedUpdatedBookDTO);

    mockMvc
        .perform(put(BOOKS_API_URL_PATH + "/" + expectedBookToUpdateDTO.getId())
                     .contentType(MediaType.APPLICATION_JSON)
                     .content(JsonConvertionUtils.asJsonString(
                         expectedBookToUpdateDTO)))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.id", is(expectedBookToUpdateDTO.getId().intValue())))
        .andExpect(jsonPath("$.name", is(expectedBookToUpdateDTO.getName())))
        .andExpect(jsonPath("$.isbn", is(expectedBookToUpdateDTO.getIsbn())));
  }

  @Test
  void whenDELETEIsCalledThenStatusNoContentShouldBeInformed()
      throws Exception {
    BookRequestDTO expectedBookToFind =
        bookRequestDTOBuilder.buildRequestBookDTO();
    ...
    doNothing()
        .when(bookService)
        .deleteByIdAndUser(any(AuthenticatedUser.class),
                           eq(expectedBookToFind.getId()));

    mockMvc
        .perform(delete(BOOKS_API_URL_PATH + "/" + expectedBookToFind.getId())
                     .contentType(MediaType.APPLICATION_JSON))
